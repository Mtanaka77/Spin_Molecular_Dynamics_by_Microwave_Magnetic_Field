...
!*       param-spinRL11.h: Basic parameters                       *
...
!*                                                               *
!% mpif90 -mcmodel=medium -fpic -O2 -o ay.out @spin_SMD11a.f03 -I/opt/fftw3/include -L/opt/fftw3/lib -lfftw3 &> log
...
!
      real(C_DOUBLE) t8_start,Temp_0,Bapz_0
...
!
      ...
      real(C_DOUBLE) wx,wy,wz,wn,wsx,wsy,wsz,wp, &
                    ...
                    ub,um,ub1,um1,uu1(3),uu2(3),ranff,ds1,ds2
...
!
      do i= i3(rank),i4(rank)  ! do 60
      l= 0
!
      do j= 1,np1+np2          ! do 62
...
!
      ...
      if(t8.gt.tmax) go to 7000
      if(wtime/60.d0.gt.cptot) go to 7000
!
!     if(kstart.ne.0) then
!       dtwr =  tau_b/80.d0
!       dtwr2 = tau_b/4.d0
...
!
        ...
        do i= 1,np1
        spx(i)= spx00(i)   ! spx() are used in the MD cycle
        spy(i)= spy00(i)
        spz(i)= spz00(i)
!
        aspx(i)= spx00(i)
...
!
      ...
      it= 0   !<<-- 
      t8= 0
      go to 1000           !<-- 1000: 288 lines above
!
!-----------------------------------------------------------
...
!
      wz7= 0
      do i= 1,np1+np2
      wz7= wz7 + (spx(i)*Bapx +spy(i)*Bapy +spz(i)*Bapz)
      end do
!
      if(wz7.gt.0.d0) then
        fw00=  1.d0    ! Choose so that ds_i/dt => -M*B < 0 
      else
        fw00= -1.d0
      end if
!
        if(rank.eq.0 .and. it.eq.1) then
          open (unit=11,file=praefixc//'.11'//suffix2, &
                    status='unknown',position='append',form='formatted')
!
          write(11,*) '% wz7(M*B)=',wz7
          close (11)
        end if
!
!   read (08,'(a40,f20.0)') text1,Bapz    ! Magnetic field Bz
      t8_start= 100.d0 ! 800.d0  ! tmax= 800 +1200.
!
...
!
        ...
        Temp= Temp_0
!           +1,-1                                        ***** -m*b < 0
        fw= fw00*(1.d0 -exp(-(t8-t8_start)/tau_b)) * sin(omg_b*(t8-t8_start))
      end if
!
      Bex= Bapx*fw   ! Bapx * sin(omg_b*t8)
      Bey= Bapy*fw
      Bez= Bapz*fw
!
      iter= 0
...
!
      ...
      J_ki= Jint(k,i)*(r_ij(k,i)/rr)**n_of_j
!
      um1= um1 - 0.5d0*J00* J_ki*(spx(i)*spx(j) +spy(i)*spy(j) &
                                                +spz(i)*spz(j))
      end do
      end do
!*
...
! 
        write(11,771) t8,it,is,iter,Usys(is),U_Fe(is),U_O(is),conv(is), &
                 e_sp1,e_c_r1,e_LJ1,magz(is),ds_Fe(is),ds_O(is), &
                 wtime/60.d0
...
!******************************************************************
!
      go to 1000                   !<-- 1000; 843 lines above
! ************* End of the Loop ***********************************
!
 7000 continue
!
