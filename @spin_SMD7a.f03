...
!   open (unit=77,771a.ps) Always in PS plots, L.255,1910,1975  
...
!
      ...
      common/parm4/ mx,my,mz
      common/parm5/ n_MCsteps,nt_p3m
      common/spins/ spin2,spin3,Jaa,Jbb,Jab,Bapx,Bapy,Bapz,Bstat, &
                    tau_b,tau_diss,Temp,TCurie
...
!
      ...
      common/ehist/ spinx(nhs),spinz(nhs),spin7(nhs),                   &
                    ...
                    timeh(nhs)
      logical       MC_first/.true./,ft06_start/.true./,if_LJ0/.true./
...
!
!
        open (unit=12,file=praefixi//'.12'//suffix1, & ! once suffix1
...
! One dimensional case
      ...
      i10(k)=     k*(np1/size) +1
      i20(k)= (k+1)*(np1/size)
      if(k.eq.size-1) i20(k)= np1
!
      disp_recv0(k)= i10(k) -1
      cnt_recv0(k) = i20(k) -i10(k) +1
      end do
!
!                  **
      do k= 0, size-1
...
!  +++++++++
        if(it.eq.1) then
          call magnetiz (spx,spy,spz,g,wx7,wy7,wz7,wn7,u1,uav7,wt7,   &
                                                          rank,i1,i2,0)
!                                              +++               Reset
...
!                          
!                                                                         Add
      call magnetiz7 (spx00,spy00,spz00,g,wx7,wy7,wz7,wn7,u1,uav7,wt7,np100,1)
! 
...
!              !<-- ksrart= 1
 3000 continue
!                                           *****
      fw= fw00*(1.d0-exp(-(t8-t8_start)/tau_b)) &
                                * sin(omg_b*(t8-t8_start))  ! -M*B < 0
      bex= Bapx*fw
      bey= Bapy*fw
      bez= Bapz*fw +Bstat*fw00
!                        +++++   ! choose the direction properly
!------------------------
!* Data transfer here!!
!------------------------
!* Local exchange of minimum data
!
      i00= i1(rank) -1
!
      iter= 0
...
!
      alp= 0.7d0
!     alp= dmin1(0.90d0, 0.50d0 +0.05d0*(iter-1)) 
!
...
!
      do i= 1,np1+np2
      vx(i)= vx(i) +dt*(fx(i) +fxC(i) +fkx(i))/mass(i)
      vy(i)= vy(i) +dt*(fy(i) +fyC(i) +fky(i))/mass(i)
      vz(i)= vz(i) +dt*(fz(i) +fzC(i) +fkz(i))/mass(i)
!
      x(i)= x(i) +dt*vx(i) 
      y(i)= y(i) +dt*vy(i)
      z(i)= z(i) +dt*vz(i)
...
!            12123456712345678123451234123456789012345678901234567890
            '  conv      e_sp      e_c_r     e_LJ',&
!            12345678901234567890123456789012345678901234567890123456
            '      magz      ds0_Fe    ds0_O    cpu(min)')
...
!
!----------------------------------------------------------------------
      subroutine magnetiz7 (spx,spy,spz,g,wx,wy,wz,wn,u1,uav,wt,np,ic)
!-----------------------------------------------------**-***-------**--
!  Magnetization
!
      use, intrinsic :: iso_c_binding
...
!
  ...
  100 continue
!
      uav= uav +u1     ! System total energy (not per atom)
      wt = wt +1
!
      if(ic.eq.7) then
        wx1= -g*wx/wn  ! in mue_B unit
        wy1= -g*wy/wn  !  -g*<spx>
        wz1= -g*wz/wn
        u1 =   uav/wt  !  <uav>
!
...
!
      return
      end subroutine magnetiz7 
!
!
!----------------------------------------**-**-**-**-*-----**---------
      subroutine magnetiz (spx,spy,spz,g,wx,wy,wz,wn,u,uav,wt,      &
                                                       rank,i1,i2,ic)
!---------------------------------------------------------------------
!  Magnetization
!
...
!
      ...
      real(C_DOUBLE) xleng0,yleng0,zleng0,t8,pi,dt,tmax,cptot,     &
                     spin2,spin3,Jaa,Jbb,Jab,Bapx,Bapy,Bapz,Bstat, &
...
!
      ...
      read (08,'(a40,f20.0)') text1,Bstat   ! Constant magnetic field Bz0
!
...
!
      real(C_float) spinx,spinz,spin7,Bextx,Bextz,magx,magy,magz, &
                    ...
                    sum_mb,U_Fe,U_O,ds_Fe,ds_O,timeh,             &
      ...
      common/ehist/ spinx(nhs),spinz(nhs),spin7(nhs), &
                    ...
                    timeh(nhs)
!
      real(C_DOUBLE) spin2,spin3,Jaa,Jbb,Jab,Bapx,Bapy,Bapz,Bstat, &
                    tau_b,tau_diss,Temp,TCurie
      common/spins/ spin2,spin3,Jaa,Jbb,Jab,Bapx,Bapy,Bapz,Bstat, &
                    tau_b,tau_diss,Temp,TCurie
      real(C_DOUBLE) t8,xleng,yleng,zleng,pi,dt,tmax,cptot
...
!
      ...
      emax = max(emax1,emax2)
      emin = 0
      call lplot1 (3,4,is,timeh,Bextx,emax1,emin1,ILN,nxtick,nytick,&
                   'Bextx.7 ',8,'        ',8,'        ',8,0)
      call lplot1 (3,5,is,timeh,Bextz,emax2,emin2,ILN,nxtick,nytick,&
...
!
      ...
      call lplot1 (3,4,is,timeh,U_Fe,emax1,emin1,ILN,nxtick,nytick,&
                   ' Kin_Fe ',8,'        ',8,'        ',8,0)
      call lplot1 (3,5,is,timeh,U_O, emax2,emin2,ILN,nxtick,nytick,&
                   ' Kin_O  ',8,'        ',8,'        ',8,0)
!------------------------
      call chart
!------------------------
